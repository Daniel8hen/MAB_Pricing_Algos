...
#       diff = (bid_price - bandit_price) > 0 # Boolean flag
    ...
    def simulate_by_constant(self, constant, discount_perc, n_iteration=100):
      """Going to simulate"""
      regret_arr = []

      for i in range(1, n_iteration):
        b = self.bid([i]) # provide a bid
        win = b > constant # win / lose accordingly

        regret_arr.append(np.abs(b - constant)) # log regret: ABS(bid - constant, which is the "win price by default")
...
lst = Parallel(n_jobs=8)(run_iteration() for i in range(8))
